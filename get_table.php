...
if(isset($_GET['stat'])){$stat = $_GET['stat'];}else{$stat = null;}
if(isset($_GET['game'])){$game = $_GET['game'];}else{$game = 0;}
if(isset($_GET['page'])){$page = $_GET['page'];}else{$page = 0;}
if(isset($_GET['row'])){$rows = $_GET['row'];}else{$rows = 100;}
if(isset($_GET['order'])){$order = $_GET['order'];}else{$order="desc";}
if(isset($_GET['active'])){$active = $_GET['active'];}else{$active="0";}
if(isset($_GET['mltp'])){$mltp = $_GET['mltp'];}else{$mltp=false;}

if(!is_numeric($active)){$active=0;}
if(!is_numeric($game)){$game=100;}
//activity
if($active==0){
...
switch($stat){
	...
	if($record_string=="user_all_record"){$stat_header="Hold Per Game <span style='font-size: 10px;'>(in seconds)</span><br><span style='font-size: 9px; font-weight:normal;'>*results inaccurate for users who played prior to Jan 2014</span>";}
	break;
	case "holdhour":
